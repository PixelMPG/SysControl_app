  ...
  <script>
    ...
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
     window.addEventListener("message", function (event) {
      renderUnidades(JSON.parse(event.data));
});
  ...
  <script>
    ...
    function handleMessage(event){
      ...
      if(ruta.length > 1){
        ...
        var firstPosition = unidades[0];
        /*var startIcon = l.icon({
          iconUrl:'https://www.flaticon.es/icono-gratis/cdn_496230',
          iconSize: [35, 35],
          iconAnchor: [16, 32],
          popupAnchor: [0, -32]
        })*/
        const firstMarker = L.marker([firstPosition.latitude , firstPosition.longitude])
        .addTo(map)
        .bindPopup("Inicio");
